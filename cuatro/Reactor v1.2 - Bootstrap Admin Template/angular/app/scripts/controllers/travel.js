...
function travelCtrl($scope, COLORS) {
  ...
  $scope.addShop = function(val) {
    ...
    shops.push({
      gridClass: grid,
      image: 'http://lorempixel.com/' + newWidth + '/' + newHeight + '?' + val,
      text: ['Cafe', 'Bar', 'Restaurant', 'Theater', 'Tuckshop'][shops.length % 5],
      name: ['The bridge', 'Millers cafe', 'Cafe Nash', 'Shangrila', 'China Gardens'][shops.length % 5]
    });
  };
  for (var i = 0; i < 28; i++) {
    $scope.addShop(i);
...
angular.module('app').controller('travelCtrl', ['$scope', 'COLORS', travelCtrl]);